...
- **Google OAuth / redirect URI**  
  Sign-in failed with `redirect_uri_mismatch`. I created a Google OAuth client, added the exact Supabase callback URL (for example: `https://smartbokkmark.supabase.co/auth/v1/callback`) and `http://localhost:3000` for local testing to the Authorized redirect URIs, then pasted the Client ID and Secret into Supabase → Authentication → Sign In / Providers → Google. Wait a few minutes for propagation and test in an incognito window.
